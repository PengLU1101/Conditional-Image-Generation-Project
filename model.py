...
class DCGAN(object):
    ...
    def build_model(self):
        ...
        self.z = tf.placeholder(
            tf.float32, [None, 64*64*3], name='uimages')
        ...
        #print('lossdim', self.complete_loss.get_shape())
        #print('zdim',self.z.get_shape())
    ...
    def train(self, config):
        ...
        assert(len(data) > 0)
        print('dim g_vars', len(self.g_vars))
        print('dim d_vars', len(self.d_vars))
...
""")
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                batch_fimages = copy.deepcopy(batch_images)
                for image in batch_fimages:
                    center = (int(np.floor(image.shape[0] / 2.)), int(np.floor(image.shape[1] / 2.)))
                    if len(image.shape) == 3:
                        #input = np.copy(image)
                        image[center[0]-16:center[0]+16, center[1]-16:center[1]+16, :] = np.random.uniform(-1, 1,size=(32, 32, 3))
                        #print(input[center[0]-16:center[0]+16, center[1]-16:center[1]+16, :])
                        target = image[center[0]-16:center[0]+16, center[1] - 16:center[1]+16, :]
                    '''else:
                        #input = np.copy(image)
                        image[center[0]-16:center[0]+16, center[1]-16:center[1]+16, :] = np.random.normal(size=(32, 32))
                        target = image[center[0]-16:center[0]+16, center[1] - 16:center[1]+16]
                        #print(image.shape)'''

                batch_z = batch_fimages.reshape((-1, 64*64*3))
                #batch_z = batch_images = np.array(batch).astype(np.float32)

                # Update D network
                _, summary_str = self.sess.run([d_optim, self.d_sum], feed_dict={ self.images: batch_images, self.z: batch_z })
                ...
                if np.mod(counter, 100) == 1:
                    samples, d_loss, g_loss = self.sess.run(
                        ...
                        feed_dict={self.z: batch_z, self.images: sample_images}#############################
                    )
...
Please first run\'./train.py --dataset ./data/train2014 --epoch 20 \'
======
...
""")
        ...
        for idx in xrange(0, batch_idxs):
            ...
            #zhats = np.random.uniform(-1, 1, size=(self.batch_size, 64*64*3))######################
            v = 0
            ...
            zhats = masked_images.reshape((-1, 64*64*3))
            for i in xrange(config.nIter*5):
                fd = {
                    self.z: zhats,#################################################
    ...
    def generator(self, z):
        ...
        #self.h0 = tf.reshape(z, [-1, 4, 4, self.gf_dim * 8])
        #self.h0, self.h0_w, self.h0_b = conv2d_transpose(z, [-1, 4, 4, self.gf_dim*8], name='g_h0', with_w=True)#_lin
        ...
        h4 = tf.nn.tanh(h4)
        #imagesample = tf.reshape(z, [self.batch_size, 64, 64, 3])
	
        return h4
        #return maskgr(imagesample, h4, self.batch_size)
    def sampler(self, z, y=None):
        tf.get_variable_scope().reuse_variables()
         
        '''h0 = conv2d_transpose(z, [-1, 4, 4, self.gf_dim*8], name='g_h0')#_lin
